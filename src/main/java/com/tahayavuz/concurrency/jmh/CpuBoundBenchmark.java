...
/**
 * Hesaplayan iş: 100 parça, her biri ~1 ms CPU.
 * <p>
 * Buradaki asıl bulgu, virtual thread'lerin <b>kazanmadığıdır</b>. Bekleme olmadığı için
 * ayrılacak bir an yoktur; iş yine çekirdek sayısı kadar paralel ilerler. Sonuç,
 * "virtual thread her yerde daha hızlıdır" varsayımının yanlış olduğunu gösterir.
 */
...
public class CpuBoundBenchmark {
    ...
    private CpuBoundWorkload workload;
    ...
    @Setup
    public void setUp() {
        workload = new CpuBoundWorkload(BenchmarkDefaults.CPU_ITERATIONS);
        sequential = new SequentialStrategy();
        fixedPool = new FixedPoolStrategy(Runtime.getRuntime().availableProcessors());
